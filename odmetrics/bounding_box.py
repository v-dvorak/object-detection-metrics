...
class ValBoundingBox:
    ...
    def __init__(self,
        ...
        ----------
            ...
            type_coordinates : Enum (optional)
                ...
                to image_size of the image. Default:'Absolute'.
            img_size : tuple (optional)
                Image image_size in the format (width, height)=>(int, int) representinh the image_size of the
    ...
    def get_relative_bounding_box(self, img_size=None):
        ...
        ----------
        img_size : tuple
            Image image_size in the format (width, height)=>(int, int)
        ...
        if img_size is None and self._width_img is None and self._height_img is None:
            raise IOError(
                'Parameter \'img_size\' is required. It is necessary to inform the image image_size.')
    ...
    def get_image_size(self):
        """ Get the image_size of the image where the bounding box is represented.

        Returns
        -------
        tupe
            Image image_size in pixels in the format (width, height)=>(int, int)
        """
    ...
    def get_coordinates_type(self):
        ...
        Enum
            Enum representing if the bounding box coordinates (x,y,w,h) are absolute or relative
                to image_size of the image (CoordinatesType.RELATIVE or CoordinatesType.ABSOLUTE).
        """
    ...
    def __eq__(self, other):
        if not isinstance(other, ValBoundingBox):
            # unrelated types
            return False
    ...
    def compare(det1, det2):
        ...
        ----------
        det1 : ValBoundingBox
            ValBoundingBox object representing one bounding box.
        dete2 : ValBoundingBox
            ValBoundingBox object representing another bounding box.
    ...
    def clone(bounding_box):
        ...
        Parameters
        ----------
        bounding_box : ValBoundingBox
            Bounding box object to be cloned.

        Returns
        -------
        ValBoundingBox
            Cloned ValBoundingBox object.
        """
        absBB = bounding_box.get_absolute_bounding_box(format=BBFormat.XYWH)
        # return (self._x,self._y,self._x2,self._y2)
        new_bounding_box = ValBoundingBox(bounding_box.get_image_name(),
    ...
    def iou(boxA, boxB):
        ...
        if ValBoundingBox.have_intersection(coords_A, coords_B) is False:
            return 0
        interArea = ValBoundingBox.get_intersection_area(coords_A, coords_B)
        union = ValBoundingBox.get_union_areas(boxA, boxB, interArea=interArea)
    ...
    def have_intersection(boxA, boxB):
        if isinstance(boxA, ValBoundingBox):
            boxA = boxA.get_absolute_bounding_box(BBFormat.XYX2Y2)
        if isinstance(boxB, ValBoundingBox):
    ...
    def get_union_areas(boxA, boxB, interArea=None):
        ...
        if interArea is None:
            interArea = ValBoundingBox.get_intersection_area(boxA, boxB)
    ...
    def get_amount_bounding_box_all_classes(bounding_boxes, reverse=False):
        ...
        ret = {}
        for c in classes:
            ret[c] = len(ValBoundingBox.get_bounding_box_by_class(bounding_boxes, c))
